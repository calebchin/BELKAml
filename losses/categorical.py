...
class MultiLabelLoss(nn.Module):
    ...
    """
    def __init__(self, epsilon: float, macro: bool, gamma: float = 2.0, nan_mask: int = 2):
        super().__init__()
    ...
    def forward(self, y_pred, y_true):
        ...
        if self.macro:
            freq = torch.stack([torch.bincount(y_true[:, i], minlength=2).float() for i in range(y_true.shape[1])], dim=0)
        else:
